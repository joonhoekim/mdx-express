...
components/
  mdx-renderer.tsx              # MDX → React 변환 진입점 (compileDocument: 본문 노드 + 목차 헤딩)
  mdx-components.ts             # 컴포넌트 레지스트리 (Writing UI + Shadcn UI)
  mdx-options.ts                # remark/rehype 플러그인 설정 (createMdxOptions: 헤딩 수집 옵션)
  mdx-error.tsx                 # MDX 에러 포맷팅 + 개발 모드 UI
  mdx-html-elements.tsx         # HTML 요소 스타일링 + 코드 블록 라우팅
  table-of-contents.tsx         # 우측 목차(TOC) — h2~h3 바로가기 + 스크롤스파이
...
hooks/                          # useIsMobile, usePanZoom, useSiblingFiles, useActiveHeading(스크롤스파이)
...
- rehype 플러그인: `rehypeKatex` → `rehypeRaw`(passThrough로 MDX 노드 5종 보존) → `rehypeSlug`(헤딩 id) → 헤딩 수집(목차용, `rehypeRaw`/`rehypeSlug` 뒤 순서 중요)
...
- `rehypeSlug`가 모든 헤딩에 `id` 부여 (한글 유지: `## 캐리 트레이드` → `id="캐리-트레이드"`)
- `compileMDX`는 VFile을 노출하지 않으므로, 헤딩은 `createMdxOptions(headings)`에 넘긴 **클로저 배열**을 rehype 플러그인이 채우는 방식으로 추출 (`compileDocument`가 `{ node, headings }` 반환)
- `TableOfContents`(클라이언트)가 h2~h3 목차 렌더 + `useActiveHeading`(IntersectionObserver)로 스크롤스파이
- 배치: 본문은 가운데 `max-w` 컬럼 유지, 목차는 우측 여백(gutter)에 absolute로 띄움. 여백이 충분할 때(`min-[1700px]`)만 표시되고, `<main>`의 `overflow-x-hidden`이 가로 스크롤 안전망
- **스크롤 구조 의존성**: 목차 sticky·사이드바 독립 스크롤은 `app/docs/layout.tsx` 최상단이 **뷰포트 높이로 고정**돼야(`h-screen`) 동작한다. `<main>`이 스크롤 컨테이너가 되고 좌측 사이드바(`ScrollArea`)가 별도 스크롤바를 가진다. ⚠️ 이 Tailwind v4 설정은 **`h-dvh` 유틸리티를 생성하지 않으므로**(높이 0 → 문서 전체가 한 덩어리로 스크롤되어 sticky/독립 스크롤 모두 깨짐) `h-screen`을 쓸 것
- ⚠️ unified 플러그인은 **attacher → transformer 2단 구조** — transformer를 직접 넘기면 unified가 그것을 attacher로 호출해 `tree`가 undefined가 된다(`Cannot use 'in' operator` 에러). 헤딩 수집기는 attacher를 반환할 것

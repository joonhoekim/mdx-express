...
subtitle: "Service는 무엇을 가져올지만 알면 된다"
description: "데이터 접근 로직을 별도 계층으로 분리하는 Repository 패턴을 다룬다. ORM 종속의 문제점, 인터페이스와 구현체 분리, TypeORM과 Prisma 교체, Decorator로 캐시 레이어 추가, Express 적용, Mock 테스트, 트레이드오프를 예제로 설명한다."

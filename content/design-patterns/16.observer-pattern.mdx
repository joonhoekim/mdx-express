...
subtitle: "한 곳의 변경이 여러 곳에 자동으로 번진다"
description: "상태 변경을 관찰자들에게 자동으로 전파하는 Observer 패턴을 다룬다. 컴포넌트 간 상태 동기화 문제, Subject·Observer 구조 구현, 장바구니 같은 실전 시나리오 적용, Redux·MobX 등 상태 관리 라이브러리와의 관계, 적용 시점을 예제로 설명한다."
---

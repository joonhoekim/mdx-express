...
ORM 모델/엔티티의 타입도 `string | null`에서 `string`(nullable: false)로 좁혀준다. drizzle이면 컬럼 정의에 `.notNull()`을 붙이는 한 줄이고(스키마가 DDL과 추론 타입을 동시에 좁힌다), 그러면 `customer.firstName` 추론 타입에서 `null`이 빠진다. 이러면 시나리오의 `.toUpperCase()` 버그는 컴파일 타임에 사라진다 — DB의 약속이 타입 시스템까지 전파되는 거다.

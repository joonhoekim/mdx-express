...
subtitle: "원본을 건드리지 않고 한 겹씩 감싼다"
description: "원본 객체를 수정하지 않고 래퍼를 겹쳐 기능을 합성하는 Decorator 패턴을 다룬다. 플래그·상속 폭발의 문제점, 다단계 알림 파이프라인 구현, 순서 제어, NestJS·프론트엔드 적용, Next.js의 한계와 함수형 합성, Redis 캐싱 조합을 예제로 설명한다."

---
title: "KDE neon 스왑 관리 완전 가이드"
subtitle: "스왑파일과 파티션, 그 사이의 모든 선택지"
description: "KDE neon과 Ubuntu 계열에서 스왑을 다루는 방법을 종합적으로 정리한다. 스왑파일과 스왑 파티션의 생성·제거·크기 조절, 두 방식 간 전환과 하이브리드 구성, 시나리오별 권장 크기, SSD 환경 최적화와 실시간 모니터링까지 다룬다."
---

import { Callout, Steps, Step, Badge } from '@/components/writing-ui'
...
echo 'UUID=<복사한-uuid> none swap sw 0 0' | sudo tee -a /etc/fstab
```
...
echo 'UUID=<hdd-partition-uuid> none swap sw,pri=10 0 0' | sudo tee -a /etc/fstab
```
...
```bash
# 작은 스왑 파티션 (2GB, 빠른 응답용)
sudo swapon -p 20 /dev/sda3

# 큰 스왑파일 (8GB, 대용량 처리용)  
sudo swapon -p 10 /swapfile_large
```
...
sudo swapon -p 10 /swapfile_backup
```
...
```bash
# HDD의 스왑을 높은 우선순위로 설정
sudo swapon -p 10 /dev/sdb2  # HDD 파티션
sudo swapon -p 5 /swapfile   # SSD 파일 (백업용)
```

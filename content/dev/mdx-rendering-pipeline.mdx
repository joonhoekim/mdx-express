...
subtitle: "파일 시스템의 .mdx가 브라우저에 닿기까지"
description: "Next.js와 next-mdx-remote로 파일 시스템 기반 문서 사이트를 구축한 과정을 다룬다. 파일 시스템·라우팅·렌더링 세 레이어의 설계, frontmatter 파싱, 컨텐츠 sanitization, remark/rehype 플러그인 체인, CJK 호환성 문제 해결과 트레이드오프를 정리한다."
...
디렉토리명에서 자동으로 제목을 생성합니다. 정렬은 `order` 필드를 우선하고, 같은 order일 경우 자연스러운 숫자 정렬(natural sort)을 적용합니다.
...
렌더링 레이어는 관심사별로 4개 파일로 분리되어 있습니다:

- `mdx-renderer.tsx` — 진입점. `compileMDX` 호출 + 에러 핸들링
- `mdx-components.ts` — 컴포넌트 레지스트리 (Writing UI + Shadcn UI + HTML 요소)
- `mdx-options.ts` — remark/rehype 플러그인 설정
- `mdx-error.tsx` — 에러 포맷팅 + 개발 모드 에러 UI
- `mdx-html-elements.tsx` — HTML 요소 스타일링 + 코드 블록 라우팅

### compileMDX 설정

`next-mdx-remote`의 `compileMDX` 함수를 사용하여 MDX 소스를 React 컴포넌트로 컴파일합니다:
...
컴파일 에러 발생 시 소스 코드 위치(줄/열)와 주변 컨텍스트를 포함한 포맷팅된 에러 메시지를 출력합니다. 개발 모드에서는 페이지에 에러 UI를 표시하고, 프로덕션에서는 에러를 다시 throw합니다.

<Callout type="info" title="주요 옵션 설명">
- **blockJS: false** — MDX 파일 내의 import/export 문을 허용합니다. 이를 통해 문서에서 직접 컴포넌트를 import할 수 있습니다.
- **remarkPlugins** — Markdown AST(mdast) 단계에서 동작하는 플러그인. CJK 문자 처리, 수식, GFM 테이블 등을 담당합니다.
- **rehypePlugins** — HTML AST(hast) 단계에서 동작하는 플러그인. `rehype-katex`로 수식을 렌더링하고, `rehype-raw`는 Markdown 안의 raw HTML을 파싱하되 MDX 노드들은 그대로 통과시킵니다.
- **format: 'mdx'** — MDX 포맷으로 파싱합니다 (일반 Markdown이 아닌 JSX가 포함된 Markdown).
- **development** — `process.env.NODE_ENV`에 따라 동적으로 설정됩니다.
</Callout>
...
flowchart LR
    ...
    B --> B2["remark-math<br/>수식 파싱"]
    B2 --> C["remark-gfm<br/>GFM 테이블/체크리스트"]
    C --> D["rehype-katex<br/>수식 렌더링"]
    D --> E["rehype-raw<br/>HTML 패스스루"]
    E --> F["React 컴포넌트"]
```

플러그인 설정은 `mdx-options.ts`에 분리되어 있습니다:
...
컴포넌트 매핑은 `mdx-components.ts`에서 관리되며, 세 가지 카테고리로 구성됩니다:
...
export const mdxComponents = {
  // Writing UI 컴포넌트들
  Callout, CodeBlock, Mermaid,
  Steps, Step,
  Tabs, TabsList, TabsTrigger, TabsContent,
  Card: WritingCard, Badge: WritingBadge,
  Blockquote, Reference, ReferenceList,
  Icon, MathCodeBridge, MugongCard,

  // Shadcn UI (접두사로 충돌 방지)
  Button,
  ShadcnCard: Card, ShadcnBadge: Badge,
  // ...
...
같은 이름의 컴포넌트가 충돌하는 경우(예: Card, Badge), Writing UI 버전에 우선순위를 두고 Shadcn UI 버전은 `Shadcn` 접두사를 붙여 구분합니다. 아이콘은 `Icon` 컴포넌트를 통해 이름으로 참조합니다.

</TabsContent>

<TabsContent value="html-elements">

`mdx-html-elements.tsx`에서 Markdown이 생성하는 기본 HTML 요소들을 Tailwind CSS로 스타일링합니다. 타입 안전한 `ComponentProps<T>` 타입을 사용합니다:
...
`pre` 요소의 매핑이 코드 블록 처리의 핵심입니다. Markdown의 코드 펜스가 파싱되면 `pre > code` 구조의 HTML이 생성되는데, 이를 가로채서 적절한 컴포넌트로 라우팅합니다. 타입 안전한 `getCodeChild()` 헬퍼를 통해 `pre > code`의 props를 추출합니다:
...
pre: (props: P<'pre'>) => {
  ...
  // 일반 코드 블록 → CodeBlock 컴포넌트
  if (childClassName.includes('language-')) {
    const language = childClassName.replace('language-', '');
    return <CodeBlock language={language}>{codeChild!.children}</CodeBlock>;
  }

  // 언어 미지정 → plaintext
  if (codeChild?.children != null) {
    return <CodeBlock language="plaintext">{codeChild.children}</CodeBlock>;
  }
...
이 패턴 덕분에 문서 작성자는 표준 Markdown 코드 펜스만 사용해도 자동으로 구문 강조, Mermaid 렌더링, 수식 렌더링이 적용됩니다.
...
Lucide React 아이콘은 `Icon` 컴포넌트를 통해 이름으로 참조합니다. `icon.tsx`에서 사용할 아이콘들을 선별적으로 import하고, `Icon` 컴포넌트가 이름을 받아 해당 아이콘을 렌더링합니다.
...
Lucide React는 수천 개의 아이콘을 제공하지만, 전부 import하면 번들 크기가 크게 증가합니다. `icon.tsx`에서 문서에서 사용할 아이콘만 선별적으로 import하고, `Icon` 컴포넌트가 이름 기반으로 렌더링합니다:

```typescript
// MDX에서 <Icon name="AlertTriangle" size={20} /> 형태로 사용
```

...
subtitle: "push 한 번으로 배포가 끝나는 파이프라인 만들기"
description: "Proxmox LXC 컨테이너에 GitHub webhook 기반 CI/CD를 구축하는 과정을 다룬다. Express webhook 서버 구현, 배포 스크립트 작성, PM2 프로세스 관리, Nginx 리버스 프록시, SSL 설정, 모니터링까지 단계별로 설명한다."

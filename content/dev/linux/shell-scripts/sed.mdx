...
subtitle: "치환 한 줄에서 로그 처리까지, 시나리오로 익히는 sed"
description: "리눅스 sed 명령어를 기본 문법부터 고급 활용까지 시나리오별로 다룬다. 텍스트 치환, 라인 선택, 정규표현식과 그룹 캡처, 다중 명령어, 로그·설정 파일·코드 리팩토링 같은 실무 활용, 대용량 파일 처리 시 주의사항을 설명한다."

...
subtitle: "저장하는 놈과 읽어오는 놈을 다른 함수로 만든다"
description: "명령(Command)과 조회(Query)의 책임을 분리하는 CQRS 패턴을 다룬다. 분리가 필요한 이유, 코드/DTO/저장소 단계별 적용, Next.js Server Actions와 조회 전용 뷰 테이블, 동기화 전략, Event Sourcing과의 관계, 오버엔지니어링 판단 기준을 예제로 설명한다."

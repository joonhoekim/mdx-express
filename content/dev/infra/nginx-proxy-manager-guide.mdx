...
subtitle: "클릭 몇 번으로 끝나는 SSL과 리버스 프록시 관리"
description: "Proxmox에 LXC 컨테이너를 만들어 Nginx Proxy Manager를 설치하고 운영하는 전 과정을 다룬다. Docker 설치, NPM 구성, 프록시 호스트와 Let's Encrypt SSL 설정, 접근 제어와 스트림, 백업·복원, 문제 해결을 설명한다."

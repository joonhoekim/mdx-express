...
채팅/대화 형태(메신저 회상, AI 챗 인용, 인터뷰 트랜스크립트)를 보여주는 컴포넌트입니다.

**두 가지 variant:**

- `bubble` (기본) — 질문(우측)은 말풍선, 답변(좌측)은 본문처럼 표시. 질문 → 답변 경계마다 가는 수평선이 자동 삽입됨 (Claude/AI 챗 스타일).
- `transcript` — 화자 헤더(●) + 본문 단락. 양쪽 모두 좌측 정렬. 인터뷰·대담에 적합.
...
>
  <Message from='me'>Next.js 16 변경점 알려줘</Message>
  <Message from='gpt'>주요 변경점은 다음과 같습니다...</Message>
</Chat>
```

**자동 동작:**

- 1:1 대화는 별도 설정 없이 동작 — 1번째 화자가 오른쪽(질문), 2번째가 왼쪽(답변)으로 자동 배치
- `color` 미지정 시 등장 순서대로 팔레트(blue → emerald → violet → amber → rose) 라운드로빈
- 같은 화자 연속 메시지는 이름 라벨이 한 번만 표시되고 말풍선 모서리가 살짝 합쳐짐
- 컨테이너는 `max-w-[1200px]`로 캡 + 가운데 정렬 (본문이 넓어도 채팅창은 적정 폭 유지)
...
<Chat
  variant='transcript'
  speakers={{
    host: { name: '진행자', color: 'blue' },
    guest: { name: '김개발', color: 'rose' },
  }}
>
  <Message from='host'>가장 큰 도전은 무엇이었나요?</Message>
  <Message from='guest'>사실 가장 어려웠던 건 사람과의 협업이었어요...</Message>
</Chat>
```
...
- `name` (필수) — 화자 표시 이름
- `side` — `'left' | 'right'`, 미지정 시 자동 (1:1일 때 1번째 → right). bubble variant에서만 의미 있음 (transcript는 무시).
- `color` — `'blue' | 'emerald' | 'violet' | 'amber' | 'rose'`, 미지정 시 자동

**Message 본문:** 일반 마크다운(리스트, 코드 블록, 강조, 링크 등) 모두 사용 가능.

**알 수 없는 화자 id:** `from`이 `speakers`에 없으면 회색 fallback으로 렌더되고 개발 모드에서 콘솔에 1회 경고 출력 (페이지는 정상 작동).

### 12. Chart (BarChart / LineChart / PieChart)

문서 안에서 간단한 통계 차트를 그릴 수 있는 컴포넌트 3종. recharts 기반.

**공통 동작:**

- 색상 미지정 시 등장 순서대로 팔레트(blue → emerald → violet → amber → rose) 자동 배정
- 시리즈/슬라이스가 2개 이상이면 범례 자동 표시, 1개면 자동 숨김
- 범례·툴팁의 라벨은 동일한 `ChartConfig`를 참조 (진실의 소스 하나)
- 그리드·툴팁 항상 표시, 다크 모드 자동
- 기본 높이 320px, `height` prop으로 조정

**BarChart / LineChart** — 다중 시리즈 지원

```mdx
<BarChart
  data={[
    { name: '1월', sales: 30, profit: 12 },
    { name: '2월', sales: 45, profit: 18 },
  ]}
  series={[
    { key: 'sales', label: '매출' },
    { key: 'profit', label: '이익', color: 'emerald' },
  ]}
  xLabel='월'
  yLabel='$ (Million)'
/>
...
<LineChart
  ...
  series={[{ key: 'users', label: '주간 활성 사용자' }]}
/>
```

**Props (BarChart, LineChart 공통):**

- `data` (필수) — `Array<Record<string, string | number>>`
- `series` (필수) — `Array<{ key, label?, color? }>`
- `xKey` — x축으로 쓸 데이터 키, 기본 `'name'`
- `xLabel` — x축 라벨 (단위·이름 표시)
- `yLabel` — y축 라벨 (단위·이름 표시)
- `height` — 기본 `320`
- `className`
...
- `data` (필수) — `Array<{ name, value, color? }>`
- `height` — 기본 `320`
- `className`

**색상 옵션:** `'blue' | 'emerald' | 'violet' | 'amber' | 'rose'` (Chat 컴포넌트와 동일 팔레트).

**비범위:** stacked bar, smooth line, donut, 값 포맷터, reference line 등은 노출하지 않음. 복잡한 차트가 필요하면 recharts를 직접 사용할 것.

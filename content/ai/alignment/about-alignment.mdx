...
subtitle: "AI의 실패는 낯설지 않다 — 인간 조직이 먼저 겪은 문제들이니까"
description: "AI의 실패 패턴(Sycophancy, Hallucination, Reward Hacking, Deceptive Alignment 등)과 이를 해결하려는 방법론(RLHF, DPO, Constitutional AI, Interpretability, Scalable Oversight), 그리고 AI Safety와 Alignment의 차이를 인간 조직의 유사 패턴과 함께 설명한다."
order: 1
...
수학적으로는 다음 토큰의 조건부 확률 $P(x_t \mid x_1, \ldots, x_{t-1})$을 최대화하는 방향으로 가중치(weight)를 업데이트함. 가중치는 모델의 "기억"이자 "판단 기준" 전체를 담고 있는 수십억~수천억 개의 부동소수점 숫자임.
...
$$
\mathbb{E}[R(x, y)] - \beta \cdot \text{KL}[\pi_\theta(y \mid x) \| \pi_{\text{ref}}(y \mid x)]
$$

여기서 첫 항은 보상 극대화, 두 번째 항은 원래 모델(reference policy)에서 너무 멀어지지 않도록 하는 KL 패널티임. $\beta$가 이 균형을 조절함.
...
DPO는 이를 단순화해서, 선호 쌍 $(y_w \succ y_l \mid x)$을 직접 손실 함수로 변환함:

...
subtitle: "두 패턴을 엮어 실무 전자결재를 완성하다"
description: "전자결재 시스템을 예제로 Saga 패턴과 Chain of Responsibility 패턴을 조합하는 방법을 다룬다. 금액별 결재선·반려·부재 대응·긴급 결재 요구사항, 두 패턴의 역할 분담, 프로세스 흐름 제어와 다단계 승인 체인 구현, 상태 관리와 알림 처리를 설명한다."
---

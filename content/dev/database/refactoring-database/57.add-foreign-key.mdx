...
<Callout type="note" title="여긴 96번의 반례다 — 제약은 단일 소유라도 DB에 둔다">
[뷰 도입](/docs/dev/database/refactoring-database/96.introduce-view)에선 "소비자가 하나면 정의를 코드로 모으라"고 했다. FK는 그 결정 규칙의 **반대편**에 선다. 차이는 "로직이냐 제약이냐"다.

- "활성 고객의 정의", "소프트 삭제 필터", "파생 합계" 같은 건 **비즈니스 로직**이다. 바뀌고, 테스트가 필요하고, 타입이 흐른다 — 단일 소유 서비스면 코드로.
- FK가 강제하는 "이 값이 저쪽에 존재하는가"는 **불변 제약**이다. 거의 안 바뀌고, 쓰기당 비용이 거의 없고, drizzle도 스키마의 `references()`로 선언하길 권한다. 이건 소비자가 하나여도 DB에 두는 게 맞다 — 앱에서 `SELECT 1`로 흉내 내면 동시성 틈(check-then-insert race)까지 떠안는다.

즉 96번 lean을 "DB엔 아무것도 두지 마라"로 과적용하지 마라. **로직은 코드로, 무결성 제약은 DB로.** 단일 소유 drizzle 서비스라면 FK는 스키마에 선언하는 게 기본값이다.
</Callout>

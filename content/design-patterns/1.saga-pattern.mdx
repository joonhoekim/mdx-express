...
subtitle: "중첩 try-catch 지옥에서 빠져나오는 보상 트랜잭션의 기술"
description: "여러 단계를 거치는 비즈니스 프로세스에서 중간 실패 시 이전 단계를 되돌리는 Saga 패턴을 다룬다. try-catch 지옥의 문제점, NestJS 기반 Saga Orchestrator와 Step 구현, 보상 트랜잭션 처리, 이벤트 기반 Saga, 트레이드오프를 예제로 설명한다."

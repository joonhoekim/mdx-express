...
subtitle: "같은 목적, 다른 방법 — 갈아 끼우는 알고리즘"
description: "같은 목적을 달성하는 여러 알고리즘을 캡슐화해 런타임에 교체하는 Strategy 패턴을 다룬다. if-else 지옥의 문제점, 결제 수단·포인트 적립·파일 업로드 전략 구현, Factory 패턴과의 조합, 적용 시점과 피해야 할 경우를 예제로 설명한다."

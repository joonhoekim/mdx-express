...
<Callout type="warning" title="drizzle엔 ORM 레벨 cascade가 없다 — 선택지는 둘뿐">
위 `orphanRemoval`/`cascade=remove`는 JPA·TypeORM처럼 **상태를 추적하는** ORM 얘기다. drizzle은 의도적으로 그런 런타임 cascade를 제공하지 않는다(`references()`의 `onDelete: 'cascade'`는 ORM 기능이 아니라 **DB의 `ON DELETE CASCADE` DDL을 생성**하는 것일 뿐이다). 그래서 drizzle 서비스가 DB를 소유할 때 현실적 선택지는 둘로 좁혀진다.

1. **DB `ON DELETE CASCADE`** — drizzle 스키마의 `onDelete: 'cascade'`로 선언하면 마이그레이션이 DDL을 찍어준다. 어느 경로로 지우든 DB가 보장(방법 A).
2. **트랜잭션 안 명시적 삭제** — 자식부터 순서대로 `db.delete(...)`를 한 트랜잭션에 묶는다. 삭제 규칙이 코드에 그대로 드러나 읽기·테스트가 쉽다.

즉 "ORM이 알아서 cascade" 환상은 drizzle엔 없다. 그래서 단일 소유 서비스라면 보통 **DB CASCADE(선언적)** 아니면 **트랜잭션 명시 삭제(코드)** 중 하나를 의식적으로 고르게 되고, 이게 오히려 DB cascade와 ORM cascade를 동시에 켜는 그 흔한 함정을 구조적으로 막아준다.
</Callout>
